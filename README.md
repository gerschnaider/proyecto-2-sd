# Reporte de Proyecto N°2: Consorcio de Noticias

### Sistemas Distribuidos (2do Cuatrimestre 2026)

**Universidad Nacional del Sur (UNS)**

---

### 👥 Integrantes y Asignación de Servicios

| Integrante                        | Rol / Servicio Asignado                | Carpeta de Microservicio  |
| :-------------------------------- | :------------------------------------- | :------------------------ |
| **Leonel Vidal**                  | Administrar suscripción de clientes    | `new_subscriptions`       |
| **Gonzalo Alfredo Aguirre**       | Enviar noticias a clientes suscritos   | `send-news`               |
| **Sabrina Barrionuevo**           | Recibir e ingestar nuevas noticias     | `servicio_recepcion`      |
| **Gerónimo Infante**              | Borrado lógico individual de noticias  | `delete-news`             |
| **Germán Schnaider**              | Gestión y ABM de áreas de noticias     | `area-manager`            |
| **Gonzalo Garcia**                | Consultar noticias de las últimas 24hs | `last-news-24-hour`       |
| **Manuel Tauro**                  | Buscar noticias por descriptor (FTS)   | `find-news-by-descriptor` |
| **Laureano Gandrup**              | Buscar noticias por rango de período   | `find-news-period`        |
| **Fernando Champredonde Brandan** | Obtener estadísticas de carga por área | `get-news-load-by-area`   |
...
- Tolerancia a fallos mediante mecanismos de **Retry con Exponential Backoff**. Si PostgreSQL detecta un fallo de serialización por carrera de condiciones o un _deadlock_ (códigos `40001` o `40P01`), el microservicio intercepta el error, aborta y reintenta la transacción automáticamente de forma transparente para el usuario.
...
- **Rol:** API REST (FastAPI) que realiza búsquedas semánticas de noticias a partir de palabras clave (descriptores), utilizando _PostgreSQL Full-Text Search_ y omitiendo registros con borrado lógico.
- **Puerto Externo:** `8030`
...
**Modo de Uso:**
Para disparar la batería de pruebas contra la IP del clúster:
